...
class Model(object):
    ...
    def back_propagate(self, input_batch, activations, loss_derivative):
        ...
        output_bias_grad = np.dot(np.ones(loss_derivative.shape[0]).T, loss_derivative)
        hid_bias_grad = np.dot(np.ones(hid_deriv.shape[0]).T, hid_deriv)
        ############################################################################
...
print "who will it"
model.predict_next_word('who', 'will', 'it')

print " Not even one more ?"
model.predict_next_word('not', 'even', 'one')

print "It is this year ."
model.predict_next_word('it', 'is', 'this')
