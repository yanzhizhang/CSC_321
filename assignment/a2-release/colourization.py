...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        ############### YOUR CODE GOES HERE ###############
        # return F.conv2d(input, self.weight, self.bias, dilation=1, padding=self.padding)
        row,column = input.size()[2:]
        dilated_weight = F.upsample(self.weight, scale_factor = self.dilation + 1 , mode='nearest')
        counter = 0
        for i in range(dilated_weight.size()[2]):
            if counter != self.dilation:
                dilated_weight[:,:,i,:] = 0
            else:
                counter = 0

        dilated_weight = dilated_weight[:, :, self.dilation:, self.dilation:]
        m =  F.conv2d(input, nn.Parameter(dilated_weight.data), self.bias, padding=(dilated_weight.size()[2])//2)
        return m
        ###################################################
