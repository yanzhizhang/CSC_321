...
class DCGenerator(nn.Module):
    def __init__(self, noise_size, conv_dim):
        ...
        self.deconv1 = deconv(noise_size,4*conv_dim,4,stride=1,padding=0,batch_norm=True)
        self.deconv2 = deconv(4*conv_dim,2*conv_dim,4,batch_norm=True)
        self.deconv3 = deconv(2*conv_dim,conv_dim,4,batch_norm=True)
...
class CycleGenerator(nn.Module):
    ...
    def __init__(self, conv_dim=64, init_zero_weights=False):
        ...
        self.conv1 = conv(3,conv_dim,4,batch_norm=True,init_zero_weights=init_zero_weights)
        self.conv2 = conv(conv_dim,2*conv_dim,4,batch_norm=True,init_zero_weights=init_zero_weights)

        # 2. Define the transformation part of the generator
        self.resnet_block = ResnetBlock(2*conv_dim)

        # 3. Define the decoder part of the generator (that builds up the output image from features)
        self.deconv1 = deconv(2*conv_dim,conv_dim,4,batch_norm=True)
...
class DCDiscriminator(nn.Module):
    ...
    def __init__(self, conv_dim=64):
        ...
        self.conv2 = conv(conv_dim,2*conv_dim,4,batch_norm=True)
        self.conv3 = conv(2*conv_dim,4*conv_dim,4,batch_norm=True)
        self.conv4 = conv(4*conv_dim,1,4,stride=1,padding=0,batch_norm=False)
